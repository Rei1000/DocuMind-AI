        ...
        with engine.begin() as conn:
            ...
            if 'upload_method' not in existing_columns:
                ...
                conn.execute(text("""
                    ...
                    CHECK (upload_method IN ('ocr', 'visio'))
                """))
                conn.execute(text("COMMENT ON COLUMN documents.upload_method IS 'Upload-Verarbeitungsmethode: ocr oder visio'"))
            ...
            if 'validation_status' not in existing_columns:
                ...
                """))
                conn.execute(text("COMMENT ON COLUMN documents.validation_status IS 'Validierungsstatus für Visio-Methode: VERIFIED, REVIEW_REQUIRED'"))
            ...
            if 'structured_analysis' not in existing_columns:
                ...
                """))
                conn.execute(text("COMMENT ON COLUMN documents.structured_analysis IS 'JSON-strukturierte Analyse-Daten der Visio-Methode'"))
            ...
            if 'prompt_used' not in existing_columns:
                ...
                """))
                conn.execute(text("COMMENT ON COLUMN documents.prompt_used IS 'Verwendeter Prompt bei Visio-Verarbeitung'"))
            ...
            if 'ocr_text_preview' not in existing_columns:
                ...
                """))
                conn.execute(text("COMMENT ON COLUMN documents.ocr_text_preview IS 'OCR-Text-Vorschau für Benutzer-Review'"))
