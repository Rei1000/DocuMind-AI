"""
Final Working Login Test
Testet Login mit Test-Framework DB (funktioniert!)
"""
...
class TestLoginFinalWorking:
    """Testet Login mit Test-Framework DB"""
    
    def test_login_final_working(self):
        """Testet Login mit Test-Framework DB"""
        print("Teste Login mit Test-Framework DB...")
        
        # ENV setzen
        set_auth_env_for_tests()
        
        # Test-Framework DB erstellen
        test_db_path = "/Users/reiner/Documents/DocuMind-AI/.tmp/test_qms_mvp.db"
        result = make_fresh_db_at(test_db_path)
        print(f"[RUNTIME] Test-Framework DB: {test_db_path}, result: {result}")
        
        # Seeds in Test-Framework DB ausführen
        try:
            conn = sqlite3.connect(test_db_path)
            user_id = seed_user_with_password(conn, "test@example.org", "TestPassword123!")
            conn.close()
            print(f"[SEEDS] test user in Test-Framework DB gesetzt: ID {user_id}")
        ...
        # Test erfolgreich
        assert True, "Login final working test completed"
